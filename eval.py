...
# from langchain_core.runnables import RunnableSequence  # removed - unused
import PyPDF2 as _maybe_PyPDF2
try:
    import PyPDF2
except Exception:
    PyPDF2 = None
...
def load_eval_history():
    ...
    return db_utils.load_eval_history()

def save_eval_history(history):
    """Save evaluation history to database"""
    db_utils.save_eval_history(history)


def load_feedback():
    """Load feedback entries from database"""
    return db_utils.load_feedback()


def save_feedback(feedback):
    """Save feedback entries to database"""
    db_utils.save_feedback(feedback)


def add_feedback_entry(username: str, message: str, role: str = None, skills: list | None = None):
    """Append a feedback entry for a user with timestamp and context."""
    db_utils.add_feedback_entry(username, message, role, skills or [])
...
def request_rerun():
    """Request a Streamlit rerun in a compatible way across versions.

    Tries `st.experimental_rerun()` first, falls back to `st.script_request_rerun()`.
    If neither is available, sets a session flag and calls `st.stop()` to
    halt the current script — the UI will still reflect the updated
    `st.session_state` values on the next manual interaction.
    """
    try:
        # Preferred in older Streamlit versions
    ...
    try:
        ...
        if hasattr(st, 'script_request_rerun'):
            return st.script_request_rerun()
    except Exception:
        pass
    # Best-effort fallback: mark a flag and stop execution
    try:
    ...
    try:
        ...
        # Last resort: raise SystemExit to abort script
        raise SystemExit()
...
def parse_resume(file_bytes: bytes, filename: str) -> Dict[str, str]:
    """Attempt to extract name, email, experience and skills from resume bytes.

    Supports PDF, DOCX, and plain text. Returns a dict with keys 'name', 'email', 'experience', 'skills'.
    Uses simple heuristics and regex; not a full NLP parser.
    """
    text = ""
    ...
    try:
        if fname.endswith('.pdf') and PyPDF2:
            ...
            # Try plain text decode for txt or unknown
            text = file_bytes.decode('utf-8', errors='ignore')
    except Exception:
        text = file_bytes.decode('utf-8', errors='ignore')
    ...
    # Heuristic for name: look for lines starting with 'Name:' or the first line with alphabetical words
    name = ""
    ...
    if not name and lines:
        # choose first short line that looks like a name (2-4 words, letters only)
        for ln in lines[:6]:
            parts = ln.split()
    ...
    # Experience heuristic: look for 'X years' or 'X+ years'
    exp = ""
    exp_match = re.search(r"(\d+)\+?\s*(?:years|yrs)", cleaned, re.I)
    if exp_match:
    ...
    # Extract skills heuristically by matching against known ROLE_SKILLS and common tokens
    text_lower = cleaned.lower()
    skill_candidates = set()
    try:
        # Flatten ROLE_SKILLS values
        all_role_skills = []
        for v in ROLE_SKILLS.values():
            all_role_skills.extend(v)
        for sk in all_role_skills:
            if sk and sk.lower() in text_lower:
                skill_candidates.add(sk)
    except Exception:
        pass

    # Common technology tokens
    common_tokens = [
        "python", "java", "c++", "c#", "c", "golang", "go", "rust", "scala",
        "docker", "kubernetes", "helm", "sql", "nosql", "react", "angular", "vue",
        "spring", "spring boot", "django", "flask", "fastapi", "terraform", "ansible",
        "kafka", "rabbitmq", "spark", "airflow", "hadoop", "maven", "gradle",
        "postgresql", "mysql", "sqlite", "mongo", "mongodb", "jpa", "hibernate",
        "rest", "api", "graphql", "html", "css", "javascript", "typescript", "node",
        "express", "bootstrap", "tailwind", "react native", "android", "ios",
        "aws", "azure", "gcp", "ci/cd", "jenkins", "git", "github", "gitlab",
        "monitoring", "prometheus", "grafana", "logging", "elastic",
        "security", "oauth", "jwt", "ssl", "tls",
        "data modeling", "etl", "data pipeline", "spark", "pandas", "numpy"
    ]
        ...
        try:
            if re.search(r"\b" + re.escape(tok) + r"\b", text_lower):
                if tok.lower() == 'sql':
                    ...
                    display = tok.title() if len(tok) > 1 else tok
                skill_candidates.add(display)
        except Exception:
            continue

    skills = sorted(skill_candidates)

    return {"name": name, "email": email, "experience": exp, "skills": skills}


def llm_parse_resume(text: str) -> Dict[str, object]:
    """Use the configured LLM to parse resume text into structured fields.

    Returns a dict with keys: name (str), email (str), experience (str), skills (list).
    Falls back to empty/partial data on failure.
    """
    ...
    try:
        template = (
            ...
            "Given the candidate resume text delimited by triple backticks, extract the following fields:"
            " name, email, overall_experience_years (number), experience_level (choose one of: Fresher, 1-3 years, 3-6 years, 6+ years), and skills (list of technical skills)."
            " Respond ONLY with a JSON object with keys: name, email, experience, skills."
            " The value for experience should be one of the levels: Fresher, 1-3 years, 3-6 years, 6+ years.\n\n"
            "Resume text:\n```{resume_text}```\n"
        )
        prompt = PromptTemplate(template=template, input_variables=["resume_text"])
        chain = prompt | llm | StrOutputParser()
        out = chain.invoke({"resume_text": text})
        # Try to extract JSON object from the output
        try:
            jtext = out[out.find("{"):out.rfind("}")+1]
            parsed = json.loads(jtext)
            # Normalize keys
            name = parsed.get('name') or parsed.get('full_name') or parsed.get('candidate_name') or ""
            email = parsed.get('email') or ""
            exp = parsed.get('experience') or parsed.get('experience_level') or ""
            skills = parsed.get('skills') or parsed.get('technical_skills') or []
            ...
            # If LLM didn't return strict JSON, attempt to parse heuristically
            # Fallback: return empty so caller can use heuristics
            return {}
    except Exception as e:
        # On any LLM error, return empty dict so caller falls back
        return {}


def mask_pii(text: str) -> str:
    """Mask common PII in free text before sending to an external LLM.

    Replaces email addresses with `[EMAIL]` and phone-like sequences
    (7+ digits, allowing separators) with `[PHONE]`. Keeps other text intact.
    """
    if not text:
        return text
    ...
    try:
        ...
        masked = re.sub(email_re, "[EMAIL]", text)
    except Exception:
        masked = text

    # Mask phone-like sequences: use a permissive pattern but only replace
    # matches that contain >=7 digits to avoid masking years or small numbers.
    try:
        ...
        masked = re.sub(phone_pattern, _phone_repl, masked)
    except Exception:
        pass
...
def home_page():
    ...
    with tab2:
        ...
        # Resume uploader: optional - place uploader outside the form so we can
        # provide an immediate 'Fill from Resume' action that copies parsed
        # values into the form fields without requiring the form to be submitted.
        use_llm = st.checkbox("Use AI resume parser (requires API key)", value=False, key="use_llm_resume")
        uploaded_resume = st.file_uploader("Upload Resume (PDF/DOCX/TXT)", type=["pdf", "docx", "txt"], key="reg_resume")
        if uploaded_resume is not None:
            parsed_flag = st.session_state.get('reg_resume_parsed', False)
            parsed_filename = st.session_state.get('reg_resume_filename')
            try:
                if not parsed_flag or parsed_filename != getattr(uploaded_resume, 'name', None):
                    if hasattr(uploaded_resume, 'getvalue'):
                        data = uploaded_resume.getvalue()
                    else:
                        uploaded_resume.seek(0)
                        data = uploaded_resume.read()
                    parsed = parse_resume(data, uploaded_resume.name)
                    # If user opted in and API key available, call the LLM to get a richer parse
                    if use_llm and API_KEY:
                        # extract raw text for LLM
                        text = ""
                        fname = (uploaded_resume.name or "").lower()
                        try:
                            if fname.endswith('.pdf') and PyPDF2:
                        ...
                        try:
                            ...
                            llm_parsed = llm_parse_resume(masked_text)
                            # Prefer LLM fields when present, fallback to heuristic
                            merged = parsed.copy()
                            if llm_parsed.get('name'):
                                merged['name'] = llm_parsed.get('name')
                            if llm_parsed.get('email'):
                                merged['email'] = llm_parsed.get('email')
                            if llm_parsed.get('experience'):
                                merged['experience'] = llm_parsed.get('experience')
                            if llm_parsed.get('skills'):
                                merged['skills'] = llm_parsed.get('skills')
                            parsed = merged
                        except Exception:
                            # If LLM parsing fails, fall back to heuristic parse
                            pass
                    # Store parsed candidates separately so we can copy them into the form
                    st.session_state.parsed_resume = parsed
                    st.session_state.reg_resume_parsed = True
                    st.session_state.reg_resume_filename = getattr(uploaded_resume, 'name', None)
                    st.success('Resume received. Click "Fill from Resume" to populate the form.')
            except Exception as e:
                st.warning(f'Resume parsing failed: {e}. You can still fill details manually.')

        # Button to copy parsed resume fields into the registration form inputs
        if st.button("Fill form from uploaded resume", key="fill_from_resume"):
            parsed = st.session_state.get('parsed_resume', {})
            if parsed:
                if parsed.get('name'):
                    st.session_state.reg_name = parsed.get('name')
                if parsed.get('email'):
                    st.session_state.reg_email = parsed.get('email')
                if parsed.get('experience'):
                    st.session_state.reg_exp = parsed.get('experience')
                if parsed.get('skills'):
                    st.session_state.reg_skills = parsed.get('skills')
            else:
                st.warning('No parsed resume data available. Please upload a resume first.')
            # No rerun here — updating `st.session_state` is enough so the
            # subsequent form rendering in this same run picks up the values.
            # Calling a rerun here caused the form to disappear in some
            # Streamlit versions/environments.
        ...
        with st.form("register_form"):
            ...
            # Skills multi-select (optional) - prefilled from resume parsing if available
            base_skills = {s for v in ROLE_SKILLS.values() for s in v}
            # If parsing produced candidate skills, include them so they can be selected
            parsed_skills_for_options = set(st.session_state.get('reg_skills', []))
            available_skills = sorted(base_skills | parsed_skills_for_options)
            default_reg_skills = st.session_state.get('reg_skills', [])
            new_skills = st.multiselect("Skills (optional)", options=available_skills, default=default_reg_skills, key="reg_skills")
                ...
                if not all([new_username.strip(), new_name.strip(), new_password.strip()]):
                    ...
                    if new_username in users:
                        ...
                        users[new_username] = {
                            ...
                            "skills": new_skills if new_skills else [],
        ...
        with tab3:
            ...
            if not st.session_state.admin_logged_in:
                ...
                if not user_list:
                    ...
                    # Build display labels including unresolved feedback counts
                    try:
                        feedback_all = load_feedback()
                    except Exception:
                        feedback_all = {}
                    label_to_user = {}
                    user_labels = []
                    for uname in user_list:
                        entries = feedback_all.get(uname, [])
                        unresolved = sum(1 for e in entries if not e.get('resolved'))
                        if unresolved > 0:
                            label = f"{uname} ({unresolved} open feedback)"
                        else:
                            label = uname
                        label_to_user[label] = uname
                        user_labels.append(label)

                    sel_label = st.selectbox("Select user to manage", user_labels, key="admin_sel_user")
                    sel_user = label_to_user.get(sel_label)
                    if sel_user:
                        ...
                        if search_q:
                            # Show filtered labels preserving feedback counts
                            filtered_labels = [lbl for lbl, uname in label_to_user.items() if uname in filtered]
                            if not filtered_labels:
                                st.info("No users match the filter.")
                            else:
                                sel_label_filtered = st.selectbox("Filtered users", filtered_labels, key="admin_sel_user_filtered")
                                sel_user = label_to_user.get(sel_label_filtered)
                                u = users.get(sel_user, {})
                            st.markdown(f"**User:** {u.get('name')}  •  **Username:** {sel_user}")
                            st.markdown(f"**Email:** {u.get('email','')}  •  **Experience:** {u.get('experience','')}")
                            st.markdown("---")
                        ...
                        try:
                            feedback_all = load_feedback()
                            user_feedback = feedback_all.get(sel_user, [])
                            if not user_feedback:
                                st.info("No feedback submitted by this user.")
                            else:
                                # Show newest first
                                for fb_idx, fb in enumerate(reversed(user_feedback), start=1):
                                    original_idx = len(user_feedback) - fb_idx  # index in original list
                                    title = f"{fb.get('date')} - {'Resolved' if fb.get('resolved') else 'Open'}"
                                    with st.expander(title, expanded=False):
                                        st.markdown(f"**Message:** {fb.get('message')}")
                                        st.markdown(f"**Role:** {fb.get('role')}")
                                        st.markdown(f"**Skills:** {', '.join(fb.get('skills') or [])}")
                                        st.markdown(f"**Resolved:** {'Yes' if fb.get('resolved') else 'No'}")
                                        st.markdown(f"**Admin note:** {fb.get('admin_comment') or ''}")
                                        coln, cola = st.columns([3,1])
                                        with coln:
                                            note_key = f"admin_fb_note_{sel_user}_{original_idx}"
                                            admin_note = st.text_input("Add / Update admin note", value=fb.get('admin_comment') or "", key=note_key)
                                        with cola:
                                            mark_key = f"admin_fb_resolve_{sel_user}_{original_idx}"
                                            if st.button("Mark Resolved", key=mark_key):
                                                try:
                                                    fa = load_feedback()
                                                    entries = fa.get(sel_user, [])
                                                    # find matching entry by date and message
                                                    for e in entries:
                                                        if e.get('date') == fb.get('date') and e.get('message') == fb.get('message'):
                                                            e['resolved'] = True
                                                            e['admin_comment'] = admin_note
                                                            break
                                                    save_feedback(fa)
                                                    st.success("Feedback marked resolved.")
                                                    request_rerun()
                                                except Exception:
                                                    st.error("Failed to mark resolved. Try again.")
                        except Exception:
...
choice = st.sidebar.selectbox("📋 Navigation", menu, index=_default_index, key="sidebar_nav")
...
if choice == "Home":
    ...
    if not st.session_state.function_called:
        ...
        with st.form("setup_form"):
            ...
            # Prefill skills from DB for logged-in users if available
            try:
                if st.session_state.get('logged_in') and st.session_state.get('username'):
                    if not st.session_state.get('setup_selected_skills') and not st.session_state.get('setup_other_skills'):
                        users = load_users()
                        u = users.get(st.session_state.username, {})
                        user_skills = u.get('skills', []) if u else []
                        if user_skills:
                            pre_sel = [s for s in user_skills if s in suggested]
                            pre_other = ', '.join([s for s in user_skills if s not in suggested])
                            if pre_sel:
                                st.session_state.setup_selected_skills = pre_sel
                            if pre_other:
                                st.session_state.setup_other_skills = pre_other
            except Exception:
                pass
    ...
    if st.session_state.role and st.session_state.skills:
        ...
        # Feedback textbox for users to send admin feedback during the evaluation
        st.markdown("---")
        st.subheader("Feedback for Admin (optional)")
        try:
            feedback_msg = st.text_area("If you have any issues or feedback about this evaluation, share it here:", key="eval_feedback_text", height=120)
            if st.button("Submit Feedback", key="submit_feedback"):
                if not st.session_state.get("username"):
                    st.error("You must be logged in to submit feedback.")
                else:
                    msg = st.session_state.get("eval_feedback_text", "").strip()
                    if not msg:
                        st.warning("Please enter feedback before submitting.")
                    else:
                        try:
                            add_feedback_entry(st.session_state.username, msg, role=st.session_state.role, skills=st.session_state.skills)
                            st.success("Thank you — your feedback has been submitted to the admins.")
                            # clear the textarea
                            st.session_state.eval_feedback_text = ""
                        except Exception:
                            st.error("Failed to save feedback — please try again later.")
        except Exception:
            # If anything goes wrong rendering feedback UI, continue without blocking interview
            pass
            ...
            with col1:
                ...
                # Add text-to-speech button for the question
                    if st.session_state.voice_mode:
                        # Use the components-based TTS player (rendered below) —
                        # removed the inline HTML-based TTS button to avoid duplicates.
                        st.caption("💡 Click the 'Listen' control below to hear the question read aloud")
